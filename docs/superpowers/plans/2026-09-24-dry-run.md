...
> **As built:** reviews added tests along the way, so the suite ran higher than each task's "Expected" count. The final count is 108 passed, 7 skipped, 4 xfailed. Review-driven changes beyond this plan: `dry_run` must be a JSON boolean; `dry_run_clearance_mm` must not be negative; `side_blade_insert_mm` must not exceed `CLOSE_SEAM_RETRACT_MM - CLOSE_SEAM_EXTRA_INSERT_MM` (33, after main's close-seam +7 mm was named on 2026-09-26); the frame check opens the robot client before its loop; blade-frame rotations are not bounds-checked; the real-run guard test pins 29 moves.

...
- Test fakes in `tests/test_control_dispatch.py`:
  ...
  - `_FakeRobotClient.transform_pose` records `(pose_in_frame, dest_frame)` in `.requests`. It passes poses through unchanged for `"world"` and adds `TOOL_OFFSET = (1, 2, 3)` for any other destination frame.
...
- [ ] **Step 1: Let the fake robot client fail for a named frame**

In `tests/test_control_dispatch.py`, change `_FakeRobotClient.__init__` and the start of `transform_pose` to:
...
```python
    ...
    async def transform_pose(self, pose_in_frame, dest_frame):
        ...
        if pose_in_frame.reference_frame in self.fail_frames:
            raise RuntimeError(f"frame {pose_in_frame.reference_frame} not found")
```

(Keep the rest of `transform_pose` as it is.)
...
Run: `venv/bin/python -m pytest tests/test_control_dispatch.py -k "frame" -v`
...
async def test_dry_run_checks_relative_moves_in_world_coordinates():
    # The fake transform adds TOOL_OFFSET (1, 2, 3) to tool-frame poses, so a
    # 10 mm jog along tool x lands at world x = 11.
    ctrl = _make_control(attrs={
...
Run: `venv/bin/python -m pytest tests/test_control_dispatch.py -k workspace -v`
